...
describe('My account tests', () => {
  ...
  it('navigate to my account and check My Activity', () => {
    ...
    cy.get('.flatpickr-day[aria-label="January 1, 2025"]')
      .eq(1)
      .should('be.visible') // Ensure element is visible
      .wait(500) // Add a small delay to ensure the aria-label has time to update
      .should('have.attr', 'aria-label', 'January 1, 2025') // Verify correct start date
      .click(); // Click to select the start date

    // Select the end date (October 23, 2024)
    cy.get('.flatpickr-day[aria-label="January 25, 2025"]')
      .eq(1)
      .should('have.attr', 'aria-label', 'January 25, 2025') // Verify correct end date
      .click();
            ...
            .each(($parentEl, index) => {
              cy.wrap($parentEl)
                ...
                .then($el => {
                  ...
                  if (firstPart === 1 && middlePart >= 1 && middlePart <= 25 && thirdPart === 25) {
                    ...
                    throw new Error(`Date ${dateText} is not valid for element ${index}. Expected format: 01/MM/25 where MM is between 1 and 25.`);
                  }
                });
            });
